        ...
        <body>
            <div class="d-flex" id="wrapper">
                ...
                <div class="container-fluid">
                    <h1 class="mt-4">Todos os Atendimentos</h1>
                            ...
                            <c:choose>
                                <c:when test="${autenticado.tipo eq 1}">    
                                    <c:forEach items="${requestScope.listAtendimento}" var="atd">
                                        <c:choose>
                                            <c:when test="${atd.status eq 'Aberto'}">
                                                <tr class="table-warning">
                                                    ...
                                                    <td><fmt:formatDate value="${atd.dataAtendimento}" pattern="dd/MM/yyyy HH:mm"/></td>
                                                    <td><c:out value="${atd.status}"/></td>
                                                    <td>
                                                        <c:if test="${autenticado.tipo eq 1}">
                                                            <a href="FuncionarioServlet?action=visualizarAtendimento&id=${atd.id}">Visualizar</a>/
                                                            <a href="FuncionarioServlet?action=removerAtendimento&id=${atd.id}">Remover</a>
                                                        </c:if>
                                                    </td>
                                                </tr>
                                            </c:when>
                                            <c:otherwise>
                                                <tr>
                                                    ...
                                                    <td><fmt:formatDate value="${atd.dataAtendimento}" pattern="dd/MM/yyyy HH:mm"/></td>
                                                    <td><c:out value="${atd.status}"/></td>
                                                    <td><a href="FuncionarioServlet?action=visualizarAtendimento&id=${atd.id}">Visualizar</a></td>
                                                </tr>
                                            </c:otherwise>
                                        </c:choose>
                                    </c:forEach>
                                                ...
                                                <c:choose>
                                                    <c:when test="${atd.status eq 'Aberto'}">
                                                        <tr class="table-warning">
                                                            ...
                                                            <td><c:out value="${atd.status}"/></td>
                                                            <td>
                                                                <c:if test="${autenticado.tipo eq 2}">
                                                                    <a href="FuncionarioServlet?action=resolverAtendimento&id=${atd.id}">Resolver</a>
                                                                </c:if>
                                                            </td>
                                                        </tr>
                                                    </c:when>

            ...
            <div class="d-flex" id="wrapper">
                ...
                <div class="container-fluid">
                    ...
                    <table class="table">
                        ...
                        <c:forEach var="atd" items="${requestScope.listAtendimento}">
                            <c:choose>
                                <c:when test="${now} <= (${atd.dataAtendimento} + 7)">
                                    <tr>
                                        <td>${atd.Cliente.nome}</td>
                                        <td>${atd.Produto.nome}</td>
                                        <td><fmt:formatDate var="year" value="${atd.dataAtendimento}" pattern="dd/MM/yyyy" /></td>
                                        <td>${atd.status}</td>
                                        <td><a href="ResolucaoAtendimentoServlet?id=${atd.id}">Resolver</a></td>
                                    </tr>
                                </c:when>
                                <c:otherwise>
